...
export async function exportEvents() {
  ...
  const rows = events.map(event => {
    ...
    if (event.type === 'feed') {
      ...
      const milkDetails = milkItems.map(milk => `Milk: ${milk.amount}ml`).join(', ');
      const solidsDetails = solidsItems.map(solids => `Solids: ${solids.name} (${solids.amount}mg)`).join(', ');
      details = [milkDetails, solidsDetails].filter(Boolean).join(' | ');
