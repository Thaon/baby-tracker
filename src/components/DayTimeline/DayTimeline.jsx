...
import { Plus, Pencil, Trash2, Baby, Utensils, Moon, Droplets } from 'lucide-react';
...
export default function DayTimeline({ selectedDate }) {
  ...
  const handleDeleteEvent = async (event) => {
    if (window.confirm(`Delete this ${event.type} event?`)) {
      try {
        ...
        toast.success(`${event.type.charAt(0).toUpperCase() + event.type.slice(1)} event deleted`);
        await fetchEvents();
      } catch (error) {
  ...
  const getEventDetails = (event) => {
    if (event.type === 'feed') {
      ...
      const totalSolids = solidsItems.reduce((sum, item) => sum + item.amount, 0);
      const parts = [];
      if (totalMilk > 0) parts.push(`Milk: ${totalMilk}ml`);
      if (totalSolids > 0) parts.push(`Solids: ${solidsItems.map(s => `${s.name} (${s.amount}mg)`).join(', ')}`);
    ...
    <div className="day-timeline">
      <div className="timeline-header">
        ...
        <div className="add-buttons">
          <button className="add-type-btn feed" onClick={() => handleAddEvent(EVENT_TYPES.FEED)}>
            <Plus size={16} /> Feed
          </button>
          <button className="add-type-btn nap" onClick={() => handleAddEvent(EVENT_TYPES.NAP)}>
            <Plus size={16} /> Nap
          </button>
          <button className="add-type-btn potty" onClick={() => handleAddEvent(EVENT_TYPES.POTTY)}>
            <Plus size={16} /> Potty
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="events-list">
          {events.map((event) => {
            ...
            return (
              <div key={event.id} className="event-item" style={{ borderLeftColor: EVENT_COLORS[event.type] }}>
                <div className="event-icon" style={{ backgroundColor: EVENT_COLORS[event.type] + '15', color: EVENT_COLORS[event.type] }}>
                  <IconComp size={20} />
                </div>
                <div className="event-content">
                  <div className="event-type-row">
                    <span className="event-type" style={{ color: EVENT_COLORS[event.type] }}>
                      {event.type.charAt(0).toUpperCase() + event.type.slice(1)}
                    </span>
                    <span className="event-time">{formatTime12(event.timeFrom)}</span>
                  </div>
                  <div className="event-details">{getEventDetails(event)}</div>
                ...
                <div className="event-actions">
                  ...
                  <button className="action-btn delete" onClick={() => handleDeleteEvent(event)}>
                    <Trash2 size={16} />
                  </button>
                </div>
              </div>
